            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('template.store') }}" method="POST">
                        @csrf
                        <div class="mb-6">
                            <x-input-label for="nama_template" value="Nama Template (Contoh: Surat Keterangan Domisili)" />
                            <x-text-input id="nama_template" name="nama_template" type="text" class="mt-1 block w-full" :value="old('nama_template')" required />
                            <x-input-error class="mt-2" :messages="$errors->get('nama_template')" />
                        </div>

                        <div class="mb-6">
                            <x-input-label for="isi_template" value="Isi Template" />
                            <textarea id="isi_template" name="isi_template" rows="15" class="mt-1 block w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm font-serif p-4" required>{{ old('isi_template') }}</textarea>
